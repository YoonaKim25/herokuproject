...
def post(request):
    if request.method == "POST":
        ...
        if form.is_valid(): #폼 검증 메소드
            board = form.save(commit=False) # board오브젝트를 form으로 부터 가져오지만, 실제로 DB반영은 하지 않는다.
            board.update_date=timezone.now() 
            board.save()
            return redirect('show') #url의 name을 경로 대신 입력한다.
    else:
...
def edit(request, pk):
        ...
        if request.method == "POST":
                form = BoardForm(request.POST, instance=board) #BoardForm으로 부터 받은 데이터를 처리하기 위한 인스턴스 생성
